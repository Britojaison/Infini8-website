...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  // Grid lines reveal/fade based on scan position
  // Lines appear when scan passes and fade out over time
  float trailFadeSpeed = 1.8;
  
  // Calculate distance behind the scan line (positive = behind scan, negative = ahead)
  float distBehindScan = scanZ - hit.z;
  
  // Lines are visible if scan has passed them
  float lineReveal = 0.0;
  if (distBehindScan > -0.15) {
      float revealStart = smoothstep(-0.15, 0.1, distBehindScan);
      float fadeOut = exp(-max(0.0, distBehindScan) * trailFadeSpeed);
      lineReveal = revealStart * fadeOut;
  }
  
  // Add bright glow near the scan line itself
  float scanProximityGlow = exp(-dz * dz * 6.0) * 0.8;
  lineReveal = max(lineReveal, scanProximityGlow);

  // Apply the reveal mask to the grid lines
  float lineVis = lineMask * lineReveal;
  ...
  float halo = max(gx, gy) * fade * lineReveal;
...
export const GridScan = ({
  ...
  const lookTarget = useRef<{ x: number; y: number }>({ x: 0, y: 0 });
  const tiltTarget = useRef(0);
  const yawTarget = useRef(0);

  const lookCurrent = useRef<{ x: number; y: number }>({ x: 0, y: 0 });
  const lookVel = useRef<{ x: number; y: number }>({ x: 0, y: 0 });
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      // Subtle tilt and yaw based on mouse position
      tiltTarget.current = ny * 0.1;
      yawTarget.current = nx * 0.15;
    };
    const onClick = async () => {
  ...
  }, [uiFaceActive, snapBackDelay, scanOnClick, enableGyro, mounted]);
  ...
  useEffect(() => {
    ...
    Promise.all([
    ...
    ]).then(([threeModule, postprocessingModule]) => {
      const THREE_NS = threeModule;
      THREE.current = THREE_NS;
      const { EffectComposer, RenderPass, EffectPass, BloomEffect, ChromaticAberrationEffect } = postprocessingModule;
      
      const s = THREE_NS.MathUtils.clamp(sensitivity, 0, 1);
      const skewScale = THREE_NS.MathUtils.lerp(0.03, 0.08, s);
      const tiltScale = THREE_NS.MathUtils.lerp(0.05, 0.12, s);
      const yawScale = THREE_NS.MathUtils.lerp(0.04, 0.1, s);
      const depthResponse = THREE_NS.MathUtils.lerp(0.25, 0.45, s);
      const smoothTime = THREE_NS.MathUtils.lerp(0.3, 0.15, s);
      const maxSpeed = Infinity;
      const yBoost = THREE_NS.MathUtils.lerp(1.1, 1.3, s);

      const lookTargetVec = new THREE_NS.Vector2(lookTarget.current.x, lookTarget.current.y);
      const lookCurrentVec = new THREE_NS.Vector2(lookCurrent.current.x, lookCurrent.current.y);
      const lookVelVec = new THREE_NS.Vector2(lookVel.current.x, lookVel.current.y);
      ...
      const uniforms = {
        iResolution: {
          value: new THREE_NS.Vector3(container.clientWidth, container.clientHeight, renderer.getPixelRatio())
        },
        iTime: { value: 0 },
        uSkew: { value: new THREE_NS.Vector2(0, 0) },
        uTilt: { value: 0 },
        uYaw: { value: 0 },
        uLineThickness: { value: lineThickness },
        uLinesColor: { value: srgbColor(THREE_NS, linesColor) },
        uScanColor: { value: srgbColor(THREE_NS, scanColor) },
      ...
      const camera = new THREE_NS.OrthographicCamera(-1, 1, 1, -1, 0, 1);
      const quad = new THREE_NS.Mesh(new THREE_NS.PlaneGeometry(2, 2), material);
      scene.add(quad);
      ...
      if (enablePost) {
        ...
        const chroma = new ChromaticAberrationEffect({
          offset: new THREE_NS.Vector2(chromaticAberration, chromaticAberration),
      ...
      const onResize = () => {
        renderer.setSize(container.clientWidth, container.clientHeight);
        (material.uniforms as any).iResolution.value.set(container.clientWidth, container.clientHeight, renderer.getPixelRatio());
      ...
      const tick = () => {
        ...
        lookTargetVec.set(lookTarget.current.x, lookTarget.current.y);

        const smoothed = smoothDampVec2(THREE_NS, lookCurrentVec, lookTargetVec, lookVelVec, smoothTime, maxSpeed, dt);
        lookCurrentVec.copy(smoothed);
        lookCurrent.current.x = lookCurrentVec.x;
        lookCurrent.current.y = lookCurrentVec.y;
        ...
        const skew = new THREE_NS.Vector2(lookCurrentVec.x * skewScale, -lookCurrentVec.y * yBoost * skewScale);
        (material.uniforms as any).uSkew.value.set(skew.x, skew.y);
        (material.uniforms as any).uTilt.value = tiltCurrent.current * tiltScale;
        (material.uniforms as any).uYaw.value = THREE_NS.MathUtils.clamp(yawCurrent.current * yawScale, -0.6, 0.6);

        (material.uniforms as any).iTime.value = now / 1000;
  ...
  useEffect(() => {
    if (!mounted || !THREE.current || !materialRef.current) return;
    const m = materialRef.current;
    const u = m.uniforms;
    u.uLineThickness.value = lineThickness;
    u.uLinesColor.value.copy(srgbColor(THREE.current, linesColor));
    u.uScanColor.value.copy(srgbColor(THREE.current, scanColor));
  ...
  useEffect(() => {
    if (!enableGyro || !mounted || !THREE.current) return;
    const handler = (e: DeviceOrientationEvent) => {
      if (uiFaceActive) return;
      const gamma = e.gamma ?? 0;
      const beta = e.beta ?? 0;
      const nx = THREE.current.MathUtils.clamp(gamma / 45, -1, 1);
      const ny = THREE.current.MathUtils.clamp(-beta / 30, -1, 1);
      lookTarget.current.x = nx;
      lookTarget.current.y = ny;
      tiltTarget.current = THREE.current.MathUtils.degToRad(gamma) * 0.4;
    };
    window.addEventListener('deviceorientation', handler);
    return () => {
      window.removeEventListener('deviceorientation', handler);
    };
  }, [enableGyro, uiFaceActive, mounted]);

  useEffect(() => {
    if (!enableWebcam || typeof window === 'undefined') return;
    let canceled = false;
    const load = async () => {
      try {
  ...
  useEffect(() => {
    if (!mounted || !THREE.current) return;
    let stop = false;
    let lastDetect = 0;
    const video = videoRef.current;

    const s = THREE.current.MathUtils.clamp(sensitivity, 0, 1);
    const depthResponse = THREE.current.MathUtils.lerp(0.25, 0.45, s);
    ...
    const start = async () => {
      ...
      const opts = new faceapiModule.TinyFaceDetectorOptions({ inputSize: 320, scoreThreshold: 0.5 });
      ...
      const detect = async (ts: number) => {
        ...
        if (ts - lastDetect >= 33) {
          lastDetect = ts;
          try {
            const res = await faceapiModule.detectSingleFace(video, opts).withFaceLandmarks(true);
            if (res && res.detection) {
              ...
              const look = new THREE.current.Vector2(Math.tanh(nxm), Math.tanh(nym));

              const faceSize = Math.min(1, Math.hypot(box.width / vw, box.height / vh));
              const depthScale = 1 + depthResponse * (faceSize - 0.25);
              const scaled = look.multiplyScalar(depthScale);
              lookTarget.current.x = scaled.x;
              lookTarget.current.y = scaled.y;
              ...
              let yawSignal = THREE.current.MathUtils.clamp((dR - dL) / (eyeDist * 1.6), -1, 1);
  ...
  }, [enableWebcam, modelsReady, sensitivity, mounted]);
...
function srgbColor(THREE: any, hex: string) {
  const c = new THREE.Color(hex);
  return c.convertSRGBToLinear();
}

function smoothDampVec2(THREE: any, current: any, target: any, currentVelocity: any, smoothTime: number, maxSpeed: number, deltaTime: number) {
